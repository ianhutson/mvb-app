...
export default function ResultsScreen({ route, navigation }: any) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (!session) { Alert.alert('Not logged in'); return; }
      ...
      if (scanId) {
        await supabase.from('scans').update({ restaurant_id: restaurant.id }).eq('id', scanId);
      }
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalBox}>
            ...
            <View style={styles.modalButtons}>
              <TouchableOpacity style={styles.modalCancel} onPress={() => setSaveModalVisible(false)} disabled={saving}>
                <Text style={styles.modalCancelText}>Cancel</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.modalSave} onPress={handleSave} disabled={saving}>
                {saving ? <ActivityIndicator size="small" color="#0f0f0f" /> : <Text style={styles.modalSaveText}>Save</Text>}
              </TouchableOpacity>
            </View>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  topNavButton: { color: '#555', fontSize: 14 },
  topNavButtonAccent: { color: '#f5c518', fontSize: 14, fontWeight: 'bold' },
  saveButton: { backgroundColor: '#f5c518', borderRadius: 8, padding: 14, alignItems: 'center', marginBottom: 16 },
  saveButtonText: { color: '#0f0f0f', fontWeight: 'bold', fontSize: 15 },
  savedBadge: { backgroundColor: '#1a2e1a', borderRadius: 8, padding: 14, alignItems: 'center', borderWidth: 1, borderColor: '#2d6a2d', marginBottom: 16 },
  savedBadgeText: { color: '#4caf50', fontWeight: 'bold', fontSize: 15 },
  title: { fontSize: 28, fontWeight: 'bold', color: '#fff', marginBottom: 4 },
  subtitleRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 },
  subtitle: { fontSize: 14, color: '#888', flex: 1 },
  unitToggle: { flexDirection: 'row', alignItems: 'center', gap: 4, paddingHorizontal: 10, paddingVertical: 4, backgroundColor: '#1e1e1e', borderRadius: 6 },
  unitOption: { color: '#555', fontSize: 12, fontWeight: 'bold' },
  unitOptionActive: { color: '#f5c518' },
  unitDivider: { color: '#333', fontSize: 12 },
  list: { gap: 12, paddingBottom: 40 },
  card: { backgroundColor: '#1e1e1e', borderRadius: 12, padding: 16 },
  bestCard: { backgroundColor: '#2a2200', borderWidth: 1, borderColor: '#f5c518' },
  bestBadge: { color: '#f5c518', fontSize: 12, fontWeight: 'bold', marginBottom: 6 },
  beerName: { color: '#fff', fontSize: 18, fontWeight: '600', marginBottom: 8 },
  bestBeerName: { color: '#f5c518' },
  details: { flexDirection: 'row', gap: 12, marginBottom: 8 },
  detail: { color: '#888', fontSize: 14 },
  score: { color: '#555', fontSize: 13 },
  bestScore: { color: '#f5c518' },
  modalOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.7)', justifyContent: 'center', alignItems: 'center', padding: 24 },
  modalBox: { backgroundColor: '#1e1e1e', borderRadius: 16, padding: 24, width: '100%', gap: 12 },
  modalTitle: { color: '#fff', fontSize: 20, fontWeight: 'bold' },
  modalSubtitle: { color: '#888', fontSize: 14 },
  modalInput: { backgroundColor: '#2a2a2a', color: '#fff', borderRadius: 8, padding: 14, fontSize: 16, marginTop: 4 },
